...
//   // 🔥 ESTADO NUEVO: Controla si ya empezó la prueba (Pantalla de Bienvenida)
...
//         // 2. 🔥 VERIFICAR SI YA FUE COMPLETADO
//         const history = await api.results.listByStudent("Ariana");
//         const alreadyTaken = history.some((h) => h.exam_id === id);

//         if (alreadyTaken) {
//           setExam(data); // Necesitamos datos para mostrar título
//           setIsCompleted(true); // Activamos pantalla de bloqueo
//           setLoading(false);
...
//             // Si ya había empezado antes, recuperamos todo y saltamos la bienvenida
...
//   // 3. TIMER (SOLO CORRE SI hasStarted ES TRUE)
//   useEffect(() => {
//     if (loading || !exam || !hasStarted || isCompleted) return; // 🔥 El reloj espera a que des INICIAR

//     timerRef.current = setInterval(() => {
//       setTimeLeft((prev) => {
//         const newVal = prev - 1;
//         // Guardado automático anti-apagón
...
//           handleFinishExam(true); // Se acabó el tiempo
//           return 0;
//         }
//         return newVal;
//       });
...
//   // ---------------------------------------------------------
//   // 🔥 NUEVA LÓGICA DE VALIDACIÓN (AÑADIR AQUÍ)
//   // ---------------------------------------------------------

//   // 1. Ayuda a comparar texto quitando espacios y símbolos raros
//   const normalizeText = (text) => {
//     if (!text) return "";
//     return String(text)
//       .replace(/\s+/g, "") // Quita espacios vacíos
//       .replace(/\$/g, "") // Quita signo dólar de LaTeX
//       .toLowerCase()
//       .trim();
//   };
...
//       const studentAnsRaw = answers[idx]; // Lo que marcó Ariana

//       // 1. Buscamos la opción correcta en la data de la IA
//       const options = q.answerOptions || q.options || [];
//       const correctOption = options.find(
//         (opt) => opt.isCorrect === true || String(opt.isCorrect) === "true"
//       );

//       // Log para depurar (Verás esto en la consola F12)
//       console.log(`P${idx + 1}: Tú respondiste: "${studentAnsRaw}"`);
//       if (correctOption)
//         console.log(`    La correcta es: "${correctOption.text}"`);
...
//         // A) Si encontramos la opción marcada como correcta (Lo ideal)
//         if (correctOption) {
//           const correctNorm = normalizeText(correctOption.text);
//           if (studentNorm === correctNorm) isCorrect = true;
//         }
//         // B) Plan de respaldo (por si el JSON viene diferente)
...
//     console.log(`RESULTADO FINAL: ${correctCount} de ${totalQ}`);
...
//             <div class="text-5xl mb-4">${scoreRounded >= 13 ? "🏆" : "📚"}</div>
...
//       Swal.fire(
//         "Error",
//         "Error al guardar tu nota de " + scoreRounded,
//         "error"
//       );
//     }
//   };
...
//               <Clock size={16} /> {exam.time_limit} min
//             </span>
//             <span className="flex items-center gap-1">
...
//   // --- PANTALLA DEL EXAMEN (SI YA EMPEZÓ) ---
...
//           {/* COLUMNA IZQUIERDA: Enunciado y Gráfico */}
...
//           {/* COLUMNA DERECHA: Alternativas (COMPACTAS Y EN GRID) */}
//           <div className="w-full md:w-[40%] bg-slate-50/50 p-4 md:p-6 overflow-y-auto max-h-[60vh] md:max-h-none">
//             <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">
//               Selecciona tu respuesta:
//             </h4>

//             {/* 🔥 GRID RESPONSIVE: 1 col en móvil, 2 en tablet/pc SI hay espacio, o lista compacta */}
...
//                     className={`
//                       relative group text-left px-4 py-3 rounded-xl border-2 transition-all duration-200 flex items-center gap-3
//                       ${
//                         isSelected
...
//                     <span
//                       className={`
//                       w-6 h-6 rounded-md flex items-center justify-center font-black text-xs border shrink-0
//                       ${
//                         isSelected
...
//           {/* Paginación de bolitas (Oculta en móviles muy pequeños) */}
